...
The columns behind both capabilities are mapped on `GameEntity`, not `Character`: they sit on
the shared table for every row, and an STI subtree cannot map a column its sibling also needs.
