...
| **L4a** | ✅ **done** — the engine plays a building. `PlaysTurns` replaces the discriminator list (a character by nature, plus anything its type declares playable), `Building` holds both contracts, and a structure's turn is its pool and its clock — no XP for standing still. The turn and progression columns moved to `GameEntity`, an STI subtree being unable to map a column its sibling needs | `APlayableBuildingTakesTurnsTest`: a wall gets no satellite, its type turns `playable`, it gets both, earns XP, takes a turn — no screen, no session |
