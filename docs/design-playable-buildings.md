...
| **L2** | ✅ **done** — `turns` and `progression` carry the eight fields, reached through `TurnService` / `ProgressionService`; every write still mirrors the `players` column, and `Player::get_row()` joins so the legacy read surface is untouched. Spending PI became a conditional `UPDATE` read by its affected-row count, the shape §3.2 asks of a shared pool | `TurnServiceTest`, `ProgressionServiceTest`, plus the whole suite unchanged |
...
| **L4a** | ✅ **done** — the engine plays a building. `PlaysTurns` replaces the discriminator list (a character by nature, plus anything its type declares playable), `Building` holds both contracts, and a structure's turn is its pool and its clock — no XP for standing still. The columns are carried by traits into the two classes that hold the capability, the shape `HarvestableFieldsTrait` already uses on the type side, so nothing climbs onto the trunk | `APlayableBuildingTakesTurnsTest`: a wall gets no satellite, its type turns `playable`, it gets both, earns XP, takes a turn — no screen, no session |
| L4b | the faction screen drives it: who sees it, who may act (`LockService::mayLock()` answers already), and being *touched* fires the turn | one type, end to end, through the UI |
