...
| **L3** | ✅ **done** — `isDue()` / `processDue()` answer without a session; `processIfDue()` keeps the tutorial and admin gates and delegates. The rule also stopped calling `getCoords()`, which *re-spawns* a cell-less entity on olympia | `TurnDueRuleTest`: the rule answers with `$_SESSION` empty, limbo suspends, nowhere does not |
