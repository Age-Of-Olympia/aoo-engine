...
class bbcode
{
    ...
    // custom
    const TAG_NAME_TELCONTAR   = 'telcontar';
    ...
    protected function generateTag(Tag $tag, &$html, ?Tag $openingTag = null, array $openTags = [])
    {
        ...
        switch ($tag->name) {
            ...
            case self::TAG_NAME_URL:
                if ($tag->opening) {
                    if ($tag->property) {
                        ...
                        // no js
                        if(in_array(substr($tag->property, 0, 11), array('javascript:')))
                            $tag->property = 'javascript:alert(\'not allowed\')';
            ...
            case self::TAG_NAME_SPOILER:
                ...
                break;
            case self::TAG_NAME_TELCONTAR:
                if ($tag->opening) {
                    $code = '<div class="telcontar">';
                } else {
                    $code = '</div>';
                }
                break;
