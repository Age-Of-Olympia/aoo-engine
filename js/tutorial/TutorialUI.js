        ...
        try {
            ...
            if (response.success) {
                ...
                if (response.completed) {
                    ...
                    //
                    // Set a marker so TutorialInit knows to wait longer
                    // before rendering the next step's panel — gives the
                    // freshly-spawned NPC's avatar/portrait images time
                    // to load so the panel doesn't appear before the
                    // dummy is visible on the map.
                    if (response.needs_map_refresh) {
                        sessionStorage.setItem('tutorial_npc_just_spawned', 'true');
