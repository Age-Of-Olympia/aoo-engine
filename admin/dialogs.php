...
function dialogs_render_form(?array $dialog, string $csrfToken): string
{
    ...
    return '<div class="d-flex justify-content-between align-items-center mb-3">'
        ...
        . '<div class="form-group col-md-2"><label>Nature</label>'
        . '<select class="form-control" name="kind">'
        . '<option value="interactive"' . (($isEdit && ($dialog['kind'] ?? '') === 'informative') ? '' : ' selected')
        . '>On s\'adresse à lui</option>'
        . '<option value="informative"' . (($isEdit && ($dialog['kind'] ?? '') === 'informative') ? ' selected' : '')
        . '>On le lit</option>'
        . '</select>'
        . '<small class="form-text text-muted">Marchand, tenancier — ou pancarte, inscription.</small></div>'
        . '<div class="form-group col-md-2"><label>Portée</label><div>'
        . '<label style="cursor:pointer;"><input type="checkbox" name="readable_from_afar" '
        . checked($isEdit && !empty($dialog['readable_from_afar'])) . '> Lisible de loin</label>'
        . '<small class="form-text text-muted">Sinon il faut être sur une case voisine.</small>'
        . '</div></div>'
