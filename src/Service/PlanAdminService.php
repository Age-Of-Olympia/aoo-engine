...
class PlanAdminService
{
    ...
    public function deletePreflight(string $plan): array
    {
        ...
        // Plan de résurrection des factions (JSON datas/*/factions) — défaut
        // 'olympia' comme scripts/map/triggers/rez.php : une faction sans
        // respawnPlan explicite bloque donc la suppression d'olympia
        $factionNames = [];
        foreach ((array) json()->get_all('factions') as $code => $faction) {
            if (is_object($faction) && (string) ($faction->respawnPlan ?? 'olympia') === $plan) {
                $factionNames[] = (string) ($faction->name ?? $code);
            }
        }
        if ($factionNames !== []) {
            $blockers[] = [
                ...
                'detail' => 'Plan de résurrection des factions : ' . implode(', ', $factionNames)
                    . ' — changez leur respawnPlan (JSON de faction) d\'abord.',
            ];
        }
