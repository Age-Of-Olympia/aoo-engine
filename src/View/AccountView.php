...
/**
 * Options du profil, extraites d'account.php pour être rendues soit en
 * page complète (account.php), soit en fragment dans un panneau du HUD
 * (load_account.php). Contenu déplacé tel quel ; buildOptions() reste
 * partagé avec le handler POST de bascule d'option d'account.php.
 */
final class AccountView
{
    /**
     * Construit la liste des options affichables/basculables du profil.
     *
    ...
    public static function buildOptions(Player $player): array
    {
        $options = array(
            ...
            'hideLineOfFire' => "Masquer la ligne de tir sur le plateau<br /><sup>La trajectoire (vert : dégagée, rouge : bloquée) s'affiche par clic droit ou appui long sur une case — cochez pour la désactiver totalement ; l'information de blocage reste dans le panneau de case</sup>",
    ...
    public static function render(Player $player, array $options, bool $hudPanel = false): void
    {
        ...
        foreach ($options as $k => $e) {
            ...
            if (($k == 'incognitoMode' || $k == 'anonymeMode') && $player->id >= 0) { //Option non disponible pour les PJ
                continue;
            } else {
                echo '<tr>';
            }
        ...
        echo '<script>'
            ...
            . '</script>'
            . '<script src="js/account.js?v=20260716"></script>';
    }
}
