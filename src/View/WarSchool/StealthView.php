...
class StealthView
{
    public static function render(Player $player, Player $target): void
    {
        ...
        $actions = $actionService->getActionsByCategory('stealth');
        ...
        echo '<h1>Compétences de Furtivité</h1>';
        ...
        if (empty($actions)) {
        echo '<p>Aucune compétence active de furtivité disponible.</p>';
        } else {
        ...
        if (empty($passives)) {
        echo '<p>Aucune compétence passive de furtivité disponible.</p>';
        } else {
            echo '<table border="1" align="center" class="marbre">';
            echo '<thead>
                    <tr>
            ...
            foreach ($passives as $passive) {
                ...
                $isRaceLearnable = (empty($passive->getRace()) || $player->data->race == $passive->getRace());
