...
{
    public static function renderResetPassword(): void
    {
        echo '<div><a href="index.php"><button><span class="ra ra-sideswipe"></span> Retour</button></a></div>';

        // generate new psw
        if (!empty($_GET['psw'])) {
            ResetPasswordView::renderGenerateNewPassword();
        }
        // recover psw
        else if (!empty($_POST['name']) && !empty($_POST['mail'])) {
            ResetPasswordView::renderSendUniqueCode();
        } else {
            ResetPasswordView::renderResetForm();
        }
    }

    private static function renderSendUniqueCode(): void
    {
